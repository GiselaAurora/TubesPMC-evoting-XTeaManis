...
void decipher(unsigned int pengulangan, char password[17], uint32_t const key[4]) {
    unsigned int i;
    char buffer[17];
...
void login(char* str, char* ID){
    ...
    while (fgets(line, sizeof(line), stream)) {
        sscanf(line, "%s %*s", akun);
        if (strncmp(akun, ID, 4) == 0) {
            char *ptr = strchr(line, ' '); // find the space character
            if (ptr != NULL) {
                char *pass = ptr + 1;
                sscanf(pass, "%s", password);
                if (strcmp(encipher(pengulangan, buffer, key),password)==0){
                    print = 1;
                    strcpy(namaUser,akun);
                }
            }
        }
    }
